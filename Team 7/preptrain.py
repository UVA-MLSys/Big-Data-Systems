...
def preprocess_and_train(df, df_last, labels):
    ...
    X_valid : array-like
        Preprocessed validation features selected based on the feature selection process.
    ...
    y_valid : array-like
        Labels corresponding to the preprocessed validation data.
    ...
    X_train_full, X_test, y_train_full, y_test = train_test_split(df, 
                                                                  labels, 
                                                                  test_size=0.2, 
                                                                  shuffle=True, 
                                                                  random_state=28)
    ...
    accuracy = pipeline.score(X_valid_preprocessed, y_valid)
    ...
    # Filter training, validation, and testing data to include only selected features
    X_train = X_train_preprocessed[:, selected_indices]
    X_valid = X_valid_preprocessed[:, selected_indices]
    ...
    return selected_feature_names, X_train, X_valid, X_test, y_train, y_valid, y_test