...
The intitial feature selection and index calculating model performs well at predicting the 1st place winner of the award, but struggles to predict the runner-ups. This is likely because there is not distribution associated with receiving a certain number of runner-up votes in the model. When 100 media members vote on who should win the award, they rank their Top 5, with each player getting points based on how many 1st, 2nd, 3rd, 4th, and 5th placed votes they receive. The following is how many points each place vote is worth: 1st = 10 points, 2nd = 7, 3rd = 5, 4th = 3, and 5th = 1. The player with the most aggregate points from votes is the winner. This point distribution demonstrates why the model can predict 1st place, but can't seem to allocate a point total for the remaining 4 places. For future enhancements, the model should look to predict a complete voting distribution for at least the top 10 players in consideration of the award. 

The criteria for MVP voting has changed over time, meaning any advanved metric trying to predict who should win the award will consantly need to be adjusted and taken with a grain of salt. For example, if we look at the most recent MVP race of the 2022-2023 season, Joel Embiid won the award over Nikola Jokic even though almost every other advanced aggregate stat (i.e., WS, VORP, and RAPTOR) favored Jokic. There are a few notable reasons for this, the main two being voter fatigue and narrative. Voter fatigue refers to how media voters prefer not to pick the same player to win the award many consectuive times, even if they are the most deserving. Havin won two straight years, Jokic may have been hampered by this bias from voter while going for his thrid consecutive award. As for narrative, this is where the crux of our project lies. Our goal, along with FiveThirtyEight and other data science organziations, was to add some objectivety to what is inherently a subjective topic. Voting is subjective, and can be skewed by unaccountable bias and narrative. Along with voter fatigue, media members thought about an era in which Giannis Antetokounmpo, Nikola Jokic, and Joel Embiid constantly placed in the top 5 for voting, and yet Embiid never ends up being one of the winners. The narrative of "Embiid never won it in his prime even with these stats" didn't sit too well in NBA discourse, so naturally journalists would prefer to have something new to write about. Of course, we don't know the thought process of every voter, hence why all we can do is play with the data and try our best to reach an objective conclusion. 
